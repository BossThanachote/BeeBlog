...
export const Navbar = ({ onToggleSidebar }: NavbarProps) => {
  ...
  // 🌟 เพิ่ม State สำหรับควบคุม AuthModal
  ...
  // 🌟 ฟังก์ชันดักจับการกดปุ่มเขียน Blog
  const handleWriteClick = (e: React.MouseEvent) => {
    if (!isAuthenticated) {
      e.preventDefault(); // หยุดการเปลี่ยนหน้า
      setIsWriteModalOpen(true); // เปิด Modal แทน
    }
  };
    ...
    <>
      <nav className="fixed top-0 w-full h-16 bg-white border-b border-gray-100 px-4 flex items-center justify-between z-50">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {/* 🌟 ย้ายปุ่ม "เขียน Blog" ออกมาโชว์ตลอดเวลา พร้อมดัก Event onClick */}
          <Link 
            href="/write" 
          ...
          {isAuthLoading ? (
            <div className="w-20 h-9 bg-gray-100 animate-pulse rounded-full ml-2"></div>
          ) : isAuthenticated ? (
            <>
              {/* ซ่อนปุ่มเขียน Blog แบบเก่าทิ้งไปแล้ว */}
              ...
              <div className="relative ml-1 sm:ml-2" ref={dropdownRef}>
                ...
                >
                  <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${userEmail}`} alt="Profile" />
                </div>
      ...
      <div 
        ref={mobileSearchRef}
        className={`fixed top-0 left-0 w-full bg-white z-[60] border-b border-gray-100 p-3 transition-all duration-300 transform md:hidden ${
          isMobileSearchOpen ? 'translate-y-0 opacity-100 shadow-lg' : '-translate-y-full opacity-0'
        }`}
      >
        <form onSubmit={handleSearchSubmit} className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => setIsMobileSearchOpen(false)} className="p-2 text-gray-400 hover:text-gray-600">
            <X className="w-6 h-6" />
          </button>
        </form>
      </div>

      {/* 🌟 วาง AuthModal ไว้ล่างสุด */}
      <AuthModal 
